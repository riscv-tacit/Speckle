...
if [ "$compileFlag" = true ]; then
   ...
   # Compile the trace-submit binary
   cd $submit_dir; make trace-submit; make trace-start; make trace-stop; make time-submit; cd ..;
   ...
   for b in ${benchmarks[@]}; do
      ...
      for input in "${commands[@]}"; do
         if [[ ${input:0:1} != '#' ]]; then # allow us to comment out lines in the cmd files
            ...
            cmd="./time-submit ./$(basename "${target_bin}") ${input}"
      ...
      # Generate a traced run script
      traced_run_script=${output_dir}/run_traced.sh
      echo "#!/bin/bash" > ${traced_run_script}
      echo "#This script was generated by Speckle gen_binaries.sh" >> ${traced_run_script}      
      IFS=$'\n' read -d '' -r -a commands < $build_dir/../commands/$suite_type/${b}.${input_type}.cmd || [ "${commands[0]}" ]
      workload_idx=0
      for input in "${commands[@]}"; do
         if [[ ${input:0:1} != '#' ]]; then # allow us to comment out lines in the cmd files
            ...
            workload_traced_run_script=${output_dir}/run_workload${workload_idx}_traced.sh
            echo "#!/bin/bash" > ${workload_traced_run_script}
            message="echo 'Running and tracing: ./$(basename "${target_bin}") ${input}'"
            cmd="./trace-submit ./$(basename "${target_bin}") ${input}"
            echo "$message" >> ${traced_run_script}
            echo "$message" >> ${workload_traced_run_script}
            echo "$cmd" >> ${traced_run_script}
            echo "$cmd" >> ${workload_traced_run_script}
            chmod +x $workload_traced_run_script
            workload_idx="$((workload_idx+1))"
         fi
      done
      chmod +x $traced_run_script
   
      # Generate a traced run script with DMA target
      traced_dma_run_script=${output_dir}/run_traced_dma.sh
      echo "#!/bin/bash" > ${traced_dma_run_script}
      echo "#This script was generated by Speckle gen_binaries.sh" >> ${traced_dma_run_script}
      IFS=$'\n' read -d '' -r -a commands < $build_dir/../commands/$suite_type/${b}.${input_type}.cmd || [ "${commands[0]}" ]
      workload_idx=0
      for input in "${commands[@]}"; do
         if [[ ${input:0:1} != '#' ]]; then # allow us to comment out lines in the cmd files
            ...
            workload_traced_dma_run_script=${output_dir}/run_workload${workload_idx}_traced_dma.sh
            echo "#!/bin/bash" > ${workload_traced_dma_run_script}
            message="echo 'Running and tracing with DMA target: ./$(basename "${target_bin}") ${input}'"
            cmd="./trace-submit --target dma ./$(basename "${target_bin}") ${input}"
            echo "$message" >> ${traced_dma_run_script}
            echo "$message" >> ${workload_traced_dma_run_script}
            echo "$cmd" >> ${traced_dma_run_script}
            echo "$cmd" >> ${workload_traced_dma_run_script}
            chmod +x $workload_traced_dma_run_script
            workload_idx="$((workload_idx+1))"
         fi
      done
      chmod +x $traced_dma_run_script

      # Generate a lossy traced run script with DMA target
      traced_dma_lossy_run_script=${output_dir}/run_traced_dma_lossy.sh
      echo "#!/bin/bash" > ${traced_dma_lossy_run_script}
      echo "#This script was generated by Speckle gen_binaries.sh" >> ${traced_dma_lossy_run_script}
      IFS=$'\n' read -d '' -r -a commands < $build_dir/../commands/$suite_type/${b}.${input_type}.cmd || [ "${commands[0]}" ]
      workload_idx=0
      for input in "${commands[@]}"; do
         if [[ ${input:0:1} != '#' ]]; then
            ...
            workload_traced_dma_lossy_run_script=${output_dir}/run_workload${workload_idx}_traced_dma_lossy.sh
            echo "#!/bin/bash" > ${workload_traced_dma_lossy_run_script}
            message="echo 'Running and tracing (lossy) with DMA target: ./$(basename "${target_bin}") ${input}'"
            cmd="./trace-submit --target dma --lossy ./$(basename "${target_bin}") ${input}"
            echo "$message" >> ${traced_dma_lossy_run_script}
            echo "$message" >> ${workload_traced_dma_lossy_run_script}
            echo "$cmd" >> ${traced_dma_lossy_run_script}
            echo "$cmd" >> ${workload_traced_dma_lossy_run_script}
            chmod +x $workload_traced_dma_lossy_run_script
            workload_idx="$((workload_idx+1))"
         fi
      done
      chmod +x $traced_dma_lossy_run_script

      # copy the submit binaries into the overlay directory
      cp $submit_dir/trace-submit $output_dir/trace-submit
      cp $submit_dir/trace-start $output_dir/trace-start
      cp $submit_dir/trace-stop $output_dir/trace-stop
      cp $submit_dir/time-submit $output_dir/time-submit
   done
